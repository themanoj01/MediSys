...
const DoctorDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchCurrentPatient = async () => {
      try {
        ...
        setCurrentPatientId(response.data.id); // Set the patient ID
      } catch (err) {
    ...
    const fetchDoctor = async () => {
      try {
        const doctorRes = await axios.get(
          `http://localhost:8090/api/doctors/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        const apptRes = await axios.get(
          `http://localhost:8090/api/appointments/get-by-doctor/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleSubmit = async (e) => {
    ...
    const appointmentUTC = new Date(
      localAppointment.getTime() - timezoneOffset * 60000
    );
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8090/api/appointments/book",
        {
          doctorId: doctor.id,
          patientId: currentPatientId, // Use the fetched patient ID
          appointmentDateTime,
        },
        {
          headers: {
      ...
      toast.error(
        error.response?.data ||
          "Failed to book appointment. May be this slot is already booked or temporarily unavailable. Please try another slot."
      );
            ...
            <div className="p-6 md:p-8 md:w-2/3 lg:w-3/4">
              ...
              <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <InfoCard
                  icon={<MapPin />}
                  title="Location"
                  value="MediSys Hospital, Level 3"
                />
              </div>
            </div>
          </div>
              ...
              {bookingSuccess ? (
                <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
                  ...
                  <p className="text-gray-600 mb-6">
                    Your appointment with {doctor.fullName} for {selectedDay} at{" "}
                    {selectedTime} has been submitted.
                  </p>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                          {getAvailableTimesForSelectedDay().map(
                            (time, index) => {
                              ...
                              const offset =
                                (selectedDayIndex - todayIndex + 7) % 7;
                              ...
                              const utcDate = new Date(
                                baseDate.getTime() - timezoneOffset * 60000
                              );
                              utcDate.setMilliseconds(0);

                              const isBooked = bookedTimeStamps.includes(
                                utcDate.getTime()
                              );
